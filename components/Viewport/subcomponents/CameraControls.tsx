...
export const CameraControls: React.FC<CameraControlsProps> = ({ isGCodeMode, setView }) => {
  return (
    <div className={`absolute ${isGCodeMode ? 'bottom-16' : 'bottom-4'} left-1/2 -translate-x-1/2 flex items-center gap-0.5 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-1 rounded-lg border border-slate-200/60 dark:border-slate-700/60 z-20 transition-all duration-300 shadow-xl shadow-black/5`}>
      <button 
        onClick={() => setView('iso')} 
