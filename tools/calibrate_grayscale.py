...
def main():
    ...
    time.sleep(2)  # Wait for mode switch stabilization
    ...
    try:
        for i, gray in enumerate(gray_values):
            ...
            # 3. Set gray value (loads static image)
            if not set_gray(args.ip, gray):
                continue
            
            time.sleep(0.5) # Allow image load to complete
            ...
            def measure_task():
                ...
                # Wait for exposure trigger to stabilize light
                time.sleep(0.5) 
            ...
            # Expose 2.5s
            # Note: measure_task sleeps 0.5s then starts recording
            # We trigger exposure immediately so light is ON when recording starts
