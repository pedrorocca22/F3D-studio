...
class DLPC1438:
    ...
    def read_max_led_pwm(self):
        """
        Read the hardware-defined maximum LED PWM values from register 0x5D.

        The DLPC1438 stores the max allowed PWM for each of the 3 LEDs as 10-bit
        values (2 bytes each, little-endian). This limit is typically set by the
        board/hardware designer and represents the safe operating maximum.

        Returns a dict with LED1, LED2, LED3 max PWM values (0-1023 range).
        """
        try:
            data = self.__i2c_read(0x5D, 6)  # 3 LEDs x 2 bytes each
            led1_max = data[0] | (data[1] << 8)
            led2_max = data[2] | (data[3] << 8)
            led3_max = data[4] | (data[5] << 8)

            result = {
                'LED1': led1_max & 0x3FF,  # Mask to 10-bit
                'LED2': led2_max & 0x3FF,
                'LED3': led3_max & 0x3FF
            }
            print(f"\n=== Hardware Max LED PWM (register 0x5D) ===")
            print(f"  LED1 max: {result['LED1']}/1023 ({result['LED1']/1023*100:.1f}%)")
            print(f"  LED2 max: {result['LED2']}/1023 ({result['LED2']/1023*100:.1f}%)")
            print(f"  LED3 max: {result['LED3']}/1023 ({result['LED3']/1023*100:.1f}%)")
            print(f"  Raw bytes: {[hex(b) for b in data]}")
            print(f"============================================\n")
            return result
        except Exception as e:
            print(f"WARNING: Could not read max LED PWM register 0x5D: {e}")
            return {'LED1': -1, 'LED2': -1, 'LED3': -1}

    def read_current_led_pwm(self):
        """
        Read the currently configured LED PWM values from register 0x55.

        Returns the current PWM setting for each LED (what was written to 0x54).
        """
        try:
            data = self.__i2c_read(0x55, 6)  # 3 LEDs x 2 bytes each
            led1 = data[0] | (data[1] << 8)
            led2 = data[2] | (data[3] << 8)
            led3 = data[4] | (data[5] << 8)
            return {'LED1': led1 & 0x3FF, 'LED2': led2 & 0x3FF, 'LED3': led3 & 0x3FF}
        except Exception as e:
            print(f"WARNING: Could not read LED PWM register 0x55: {e}")
            return {'LED1': -1, 'LED2': -1, 'LED3': -1}
    ...
    def configure_external_print(self, LED_PWM, skip_FPGA_video=False): 
        ...
        print(f"\nConfiguring the DLPC1438 for external print mode with LED PWM at {LED_PWM/1023*100:.1f}%")
        ...
        # Read hardware max before configuring
        hw_max = self.read_max_led_pwm()
        if hw_max['LED1'] >= 0 and LED_PWM > hw_max['LED1']:
            print(f"WARNING: Requested LED_PWM ({LED_PWM}) exceeds hardware max ({hw_max['LED1']})!")
            print(f"  Clamping to hardware max: {hw_max['LED1']}")
            LED_PWM = hw_max['LED1']
        ...
        # Read back to verify what was actually set
        current = self.read_current_led_pwm()
        print(f"LED PWM configured: {LED_PWM}/1023 | Readback: LED1={current['LED1']}, LED2={current['LED2']}, LED3={current['LED3']}")
    ...
    def set_led_pwm(self, LED_PWM):
        """
        Dynamically update only the LED PWM register (0x54) without reconfiguring
        the entire EXTERNAL_PRINT mode. Use only after configure_external_print has been called once.
        """
        assert isinstance(LED_PWM, int), "LED_PWM must be integer"
        
        # Check HW limit (optional optimization: skip read if confident, but safe is better)
        # To speed up calibration loop, we might want to skip the read_max call if we know limits?
        # Let's keep it safe.
        # hw_max = self.read_max_led_pwm() 
        # Actually, reading I2C is slow. For loop calibration, maybe just clamp blindly to 1023 or trusting caller?
        # Let's trust caller but ensure 10-bit range.
        if LED_PWM > 1023: LED_PWM = 1023
        if LED_PWM < 0: LED_PWM = 0

        # print(f"> Updating LED PWM to {LED_PWM}")
        PWM_data = list(LED_PWM.to_bytes(2, byteorder = 'little')) + [0x00, 0x00, 0x00, 0x00]
        self.__i2c_write(0x54, PWM_data)
